...
export default function RecipePage() {
  ...
  const fetchRecipe = async () => {
    try {
      ...
      if (source === 'gemini') {
        // This is a Gemini recipe - load from localStorage using the key from URL
        const storageKey = urlParams.get('key');
        console.log('🔥 Looking for Gemini recipe with key:', storageKey);
        ...
        const storedRecipe = localStorage.getItem(storageKey);
        console.log('🔥 Found in localStorage:', storedRecipe ? 'YES - ' + storedRecipe.length + ' chars' : 'NO');
        console.log('🔥 All localStorage keys:', Object.keys(localStorage).filter(k => k.startsWith('gemini_recipe_')));
        
        if (storedRecipe) {
          try {
            const parsedRecipe = JSON.parse(storedRecipe);
            console.log('✅ Gemini recipe loaded:', parsedRecipe.title);
            console.log('✅ Has YouTube video:', parsedRecipe.youtube_video_id ? 'YES (' + parsedRecipe.youtube_video_id + ')' : 'NO');
            console.log('✅ Recipe data keys:', Object.keys(parsedRecipe).join(', '));
            
            // Clean up localStorage after a delay (5 seconds) to allow for page refresh
            setTimeout(() => {
              localStorage.removeItem(storageKey);
              console.log('🗑️ Cleaned up localStorage key:', storageKey);
            }, 5000);
        ...
        // If Gemini recipe not found in localStorage, show error
        console.error("❌ Gemini recipe data not found in localStorage with key:", storageKey);
        console.error("❌ Available keys:", Object.keys(localStorage).filter(k => k.startsWith('gemini_recipe_')));
        setRecipe(null);
        setLoading(false);
        return; // Don't fall back to backend for Gemini recipes
      }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3 mb-4">
              {recipe.category && (
                <div className={`inline-block px-4 py-2 rounded-full text-sm font-semibold uppercase tracking-wider ${
                  recipe.category === 'VEGETARIAN' ? 'bg-green-500 text-white' :
                  recipe.category === 'VEGAN' ? 'bg-emerald-600 text-white' :
                  recipe.category === 'NON-VEGETARIAN' || recipe.category === 'NON-VEG' ? 'bg-red-500 text-white' :
                  recipe.category === 'EGGETARIAN' ? 'bg-yellow-500 text-white' :
                  'bg-orange-500 text-white'
                }`}>
                  {recipe.category === 'NON-VEGETARIAN' ? 'NON-VEG' :
                   recipe.category === 'VEGETARIAN' ? 'VEG' :
                   recipe.category}
                </div>
              )}
      ...
      {relatedRecipes.length > 0 && (
        <div className="bg-gray-900 py-16">
          <div className="max-w-7xl mx-auto px-4">
            <h2 className="text-4xl font-bold text-white mb-12">Other recipes</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {relatedRecipes.map((relatedRecipe, index) => (
                <div key={index} className="bg-gray-800 rounded-3xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 cursor-pointer border border-gray-700 hover:border-orange-500"
                  ...
                  <div className="p-6">
                    ...
                    <h3 className="text-xl font-bold text-white">{relatedRecipe.title}</h3>
                  </div>
                </div>
              ))}
            </div>
